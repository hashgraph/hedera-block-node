...
public final class Translator {
    ...
    @NonNull
    private static <T extends Record> byte[] asBytes(
            @NonNull final Codec<T> codec, @NonNull final T tx) {
        requireNonNull(codec);
        requireNonNull(tx);
        try (final ByteArrayOutputStream baos = new ByteArrayOutputStream();
                final WritableStreamingData wsd = new WritableStreamingData(baos)) {
            // TODO: should 'baos' and 'wsd' be used like that (autoclosed) here? Probably yes, but
            // maybe something I do not know?
            codec.write(tx, wsd);
