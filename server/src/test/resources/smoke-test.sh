...
# 2. Call the endpoints /health/livez and /health/readyz
SERVER_URL="http://localhost:8080"
LIVENESS_ENDPOINT="/healthz/livez"
READINESS_ENDPOINT="/healthz/readyz"

if ! curl -v --connect-timeout 5 --max-time 10 --retry 5 --retry-delay 0 $SERVER_URL$LIVENESS_ENDPOINT; then
...
if ! curl -v --connect-timeout 5 --max-time 10 --retry 5 --retry-delay 0 $SERVER_URL$READINESS_ENDPOINT; then
...
# 4. Start the producer script with parameter 1 and save logs to producer.log
