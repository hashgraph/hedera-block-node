...
public class BlockWriter {
    ...
    public static void writeBlock(final Path baseDirectory, final Block block) throws IOException {
        ...
        final long blockNumber = switch (firstBlockItem.item().kind()) {
            case BLOCK_HEADER -> firstBlockItem.blockHeader().number();
            case RECORD_FILE -> firstBlockItem.recordFile().number();
            default -> throw new IllegalArgumentException("Block first item is not a block header or record file");
        };
        ...
        final String zipFileNameDigits = blockNumberStr.substring( offsetToZip, offsetToZip+DEFAULT_DIGEST_PER_ZIP_FILE_NAME);
        ...
        for (int i = 0; i < directoryDigits.length(); i += DEFAULT_DIGITS_PER_DIR) {
            final String dirName = directoryDigits.substring(i,
                    Math.min(i + DEFAULT_DIGITS_PER_DIR, directoryDigits.length()));
        ...
        try (FileSystem fs = FileSystems.newFileSystem(
            ...
            try (WritableStreamingData out = new WritableStreamingData(new ZstdOutputStream(Files.newOutputStream(blockPathInZip,
     ...
     */
    record BlockPath(Path dirPath, String zipFileName, String blockFileName) { }
